...
const handlePhaseShift = (state) => {
  switch (state.phase) {
    ...
    default:
      throw Error("handlePhaseShift() called from non-betting phase");
  }
};
...
const calculateSidePots = (state, playerStacks) => {
  ...
  const investedPlayers = playerStacks.filter(
    (player) => player.sidePotStack > 0
  );
  ...
  if (investedPlayers.length === 1) {
    ...
    const playerToRefund =
      state.players[
        state.players.findIndex(
          (player) => player.name === investedPlayers[0].name
        )
      ];
    playerToRefund.chips =
  ...
  const ascBetPlayers = investedPlayers.sort(
    (a, b) => a.sidePotStack - b.sidePotStack
  );
  const smallStackValue = ascBetPlayers[0].sidePotStack;

  const builtSidePot = ascBetPlayers.reduce(
    (acc, cur) => {
      /***
  ...
  if (state.sidePots.length > 1) {
    for (let i = 0; i < state.sidePots.length; i++) {
      for (let n = i + 1; n < state.sidePots.length; n++) {
        if (
          arrayIdentical(
            state.sidePots[i].contestants,
            state.sidePots[n].contestants
          )
        ) {
          state.sidePots[i].potValue =
...
const arrayIdentical = (arr1, arr2) => {
  ...
  return arr1.map((el) => arr2.includes(el)).filter((bool) => bool !== true)
    .length !== 0
    ? false
    : true;
