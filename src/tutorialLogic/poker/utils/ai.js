...
const handleAI = (state, pushAnimationState) => {
  ...
  const descendingSortHand = activePlayer.cards
    .concat(state.communityCards)
    .sort((a, b) => b.value - a.value);
  const { frequencyHistogram, suitHistogram } =
    generateHistogram(descendingSortHand);
  ...
  switch (state.phase) {
    case "betting1": {
      ...
      const callValue =
        activePlayer.chips + activePlayer.bet >= highBet
          ? highBet
          : activePlayer.chips + activePlayer.bet;
      if (willCall) {
        if (willRaise(raiseChance)) {
          const determinedRaiseRange =
            raiseRange[Math.floor(Math.random() * (raiseRange.length - 0)) + 0];
          const wantRaise =
            BET_HIERARCHY[stakes] <= BET_HIERARCHY[determinedRaiseRange];
          if (wantRaise) {
            let betValue = Math.floor(
              decideBetProportion(determinedRaiseRange) * activePlayer.chips
            );
            if (betValue < highBet) {
              if (highBet < max) {
                betValue = highBet;
            ...
            pushAnimationState(
              ...
              `${renderActionButtonText(highBet, callValue, activePlayer)} ${
                callValue > activePlayer.bet ? callValue : ""
              }`
            );
    ...
    case "betting4":
      ...
      const flushCards =
        isFlush &&
        descendingSortHand.filter((card) => card.suit === flushedSuit);
      ...
      const highRank =
        handHierarchy[handHierarchy.findIndex((el) => el.match === true)].name;
      const { callLimit, raiseChance, raiseRange } =
...
const decideBetProportion = (stakes) => {
  if (stakes === "blind") {
    ...
    return Math.random() * (0.4 - 0.35) + 0.35;
...
const generateHistogram = (hand) => {
  const histogram = hand.reduce(
    (acc, cur) => {
      acc.frequencyHistogram[cur.cardFace] =
        (acc.frequencyHistogram[cur.cardFace] || 0) + 1;
